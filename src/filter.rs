...
pub const FILTER_OFFSET: usize = 33;
...
pub struct AudioBuffer {
    ...
    pub data_raw:  [(i32, i32); AUDIO_BUF_LENGTH + FILTER_OFFSET], //(right,left)  
}

//Init audio buffer
pub fn init_audio_buffer() -> AudioBuffer {
    AudioBuffer{data_filter: [(0,0); AUDIO_BUF_LENGTH], data_raw: [(0,0); AUDIO_BUF_LENGTH + FILTER_OFFSET]}
}

//Filter the raw audio data. Filter one signal at a time
pub fn fir_filter(audio_buf: &mut AudioBuffer, index: usize) {
    //Wie viele Werte einschließlich mit dem aktuellen für die Berechnung betrachetet werden sollen
    let mut sum_right : i32 = 0;
    let mut sum_left : i32 = 0;
    for j in 0..FILTER_OFFSET {
    ...
    audio_buf.data_filter[index - FILTER_OFFSET] = ( (sum_right as i32), (sum_left as i32));   
}