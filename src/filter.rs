...
pub const MULTIPLIER: [i64; 33] = [-399,-46,407,486,46,-525,-626,-46,731,885,046,-1185,-1532,-46,2997,6062,7349,6062,2997,-46,-1532,-1185,46,885,731,-46,-626,-525,46,486,407,-46,-399];
...
pub fn fir_filter(audio_buf: &mut AudioBuffer, index: usize) {
    ...
    for j in 0..n {
        //sum_right = sum_right + (MULTIPLIER[j] * audio_buf.data_raw[((index + AUDIO_BUF_LENGTH - j) % AUDIO_BUF_LENGTH) as usize ].0 as i64) as i64;
        //sum_left =  sum_left + (MULTIPLIER[j] * audio_buf.data_raw[((index + AUDIO_BUF_LENGTH - j) % AUDIO_BUF_LENGTH) as usize ].1 as i64) as i64;
    ...
    audio_buf.data_filter[index] = ( ((sum_right & 0xFFFFFFFF) as i32), ((sum_left & 0xFFFFFFFF) as i32));
